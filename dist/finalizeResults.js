...
async function finalizeResults(apiResults, userInputs) {
    let finalResult = {
        // split user input and save currency one in the following format
        // UAE Dirham (AED)
        base: `${userInputs.countryFrom.split(' - ')[1]} (${userInputs.countryFrom.split(' - ')[0]})`,
        // split user input and save currency two in the following format
        // UAE Dirham (AED)
        final: `${userInputs.countryTo.split(' - ')[1]} (${userInputs.countryTo.split(' - ')[0]})`,
        // get amount from user input
        base_amount: userInputs.amount,
        // get final amount by multiplying given amount and exchange rate
        final_amount: apiResults.conversion_rates[userInputs.countryTo.split(' - ')[0]] *
            userInputs.amount,
